...
$$\mathbf{a}' = \mathbf{a} + \alpha \cdot \|\boldsymbol{\mu}_\text{sp} - \boldsymbol{\mu}_\text{ns}\| \cdot \hat{\mathbf{w}}$$

where $\|\boldsymbol{\mu}_\text{sp} - \boldsymbol{\mu}_\text{ns}\|$ is the Euclidean distance between the class centre vectors, and $\alpha$ is a signed scaling factor. Setting $\alpha < 0$ pushes the non-speech activation towards the non-speech end of the direction; $\alpha > 0$ pushes it towards speech. We are interested in whether pushing a non-speech example further into the non-speech region reduces hallucinated output — i.e. whether the decoder, confronted with a more emphatic non-speech signal, produces less text.
