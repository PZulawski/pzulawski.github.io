...
The probe direction is fitted on 900 examples per class. The steering magnitude is tuned to $\alpha_\text{max} = 3.2$ — i.e. $\alpha = -3.2 \cdot (1 - t)$ — on a held-out development set. Evaluation is then run on 100 fresh examples per class, reporting only examples where the word count changes (CHANGED_ONLY).
