...
- **Case A**: a latent `is_speech` representation exists in the encoder activations, but has no causal path to the output — the decoder ignores it. The information is there; the model just doesn't report it.
...
Note that `TransformerBlock.forward` returns a tuple `(residual_stream, kv_state)` — hence `output[0]`. Mean-pooling over time is a reasonable first choice: the speech/non-speech distinction should be distributed across the whole segment, not concentrated in a single frame. We run body and encoder, but not the decoder — we are interested in what the encoder has built up, independent of how the decoder interprets it.
...
For each layer, logistic regression (L2 regularisation, `C=0.1`) is trained with 5-fold cross-validation. The fitted weight vector $\mathbf{w}$ for the best-performing layer is retained for visualisation and the causal intervention.
...
![Linear probe accuracy across all 32 encoder layers — 5-fold cross-validation. Chance is 0.5.](/images/ml/probe-accuracy-vs-depth.png)

Across all 32 layers, probe accuracy is 0.999–1.000. There is no warm-up period, no layer at which the distinction first "emerges" — it is there from layer 0 and stays there. This is a striking result. The best layer (layer 2) achieves 1.000 ± 0.001; the worst layer achieves 0.999 ± 0.001. The model encodes the speech/non-speech distinction essentially perfectly, from the very first transformer layer.

The 1D projection confirms it visually: the two distributions are completely separated, with essentially no overlap. The ambiguous zone (shaded gold) sits in the gap between the two clusters — examples that genuinely sit between the two, which turn out to be the expected edge cases (faint background chatter, distant voices) on audio inspection.

![Left: 1D projection of all examples onto the probe direction at layer 2 — complete separation. Right: PCA scatter at the same layer, with ambiguous-zone examples highlighted (gold diamonds).](/images/ml/probe-direction-viz.png)

The PCA scatter underlines this: PC1 alone accounts for 42% of the variance at this layer, and the two classes sit cleanly at opposite ends of it. On audio inspection, the examples near the boundary are genuinely ambiguous: crowd noise, intelligible background conversations, shouts and yells, lyrics within background music — content that sits in the grey area between speech and non-speech by any reasonable definition. It is also worth noting that the distribution along the probe direction is smooth, with a continuous gradation from confidently non-speech to confidently speech-like rather than a hard split. This is circumstantial but suggestive evidence that the direction genuinely encodes speech-likeness, rather than some other quantity that happens to correlate with the class label in this dataset.

## Interpretation

The probe results strongly suggest **Case A**: the encoder builds a near-perfect linear representation of the speech/non-speech distinction from the very first layer, grounded in low-level acoustic features. This representation does not change materially as it propagates through the 32-layer stack. The model unambiguously "knows" it is looking at non-speech.
